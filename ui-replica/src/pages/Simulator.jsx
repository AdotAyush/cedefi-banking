            ...
            {selectedTx && (
              <div className="mt-4 space-y-2">
                <div className="p-2 bg-muted rounded text-sm">
                  <p>
                    <strong>Status:</strong> {transactions.find((t) => t.transactionId === selectedTx)?.status}
                  </p>
                  <p>
                    <strong>Recipient Status:</strong> {transactions.find((t) => t.transactionId === selectedTx)?.recipientStatus}
                  </p>
                </div>
              ...
              return (
                ...
                >
                  ...
                  {node.status === 'ACTIVE' && selectedTx && currentTx?.status === 'PENDING' && !hasVoted && (
                    <div className="flex gap-2 mt-2">
                      <Button size="sm" variant="outline" className="flex-1 hover:bg-green-100 hover:text-green-700 border-green-200" onClick={() => handleVote(node.publicKey, true)}>
                        <FaThumbsUp className="mr-1" /> YES
                      </Button>
                      <Button size="sm" variant="outline" className="flex-1 hover:bg-red-100 hover:text-red-700 border-red-200" onClick={() => handleVote(node.publicKey, false)}>
