...
const Simulator = () => {
    ...
    // Sort nodes so voted ones float to the top in the order they voted.
    // Unvoted active nodes follow, then pending/fraudulent at the bottom.
    const voteMap = new Map(currentTx?.votes?.map(v => [v.voter, v]) ?? []);
    const sortedNodes = [...nodes].sort((a, b) => {
        const va = voteMap.get(a.publicKey);
        const vb = voteMap.get(b.publicKey);
        if (va && vb) return new Date(va.timestamp) - new Date(vb.timestamp); // voted: earliest first
        if (va) return -1;  // a voted, b didn't → a first
        if (vb) return 1;   // b voted, a didn't → b first
        return 0;           // neither voted → keep relative order
    });
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {sortedNodes.map((node, i) => {
                    ...
                    const vote        = voteMap.get(node.publicKey);
                    const hasVoted    = !!vote;
                    const voteOrder   = hasVoted
                        ? [...voteMap.values()]
                            .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
                            .findIndex(v => v.voter === node.publicKey) + 1
                        : null;
                    ...
                    return (
                        <motion.div
                            ...
                            transition={{ layout: { type: 'spring', stiffness: 300, damping: 30 }, delay: hasVoted ? 0 : i * 0.03 }}
                                ...
                                {selectedTx && hasVoted && (
                                    ...
                                    >
                                        <div className={`flex items-center gap-1.5 font-bold mb-1 ${vote.decision ? 'text-emerald-400' : 'text-red-400'}`}>
                                            ...
                                            {voteOrder && (
                                                <span className="ml-1 text-[10px] text-slate-500 font-normal">#{voteOrder}</span>
                                            )}
